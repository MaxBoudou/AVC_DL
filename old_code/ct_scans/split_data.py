...
nb_diagnosis = hemorrhage_diagnosis_df.shape[0]
print(nb_diagnosis)
for batch in range(0,25):
    print(batch)
    if batch != 24:
        hemorrhage_diagnosis_array = hemorrhage_diagnosis_df[batch*100: batch*100+100]._get_values
    else:
        hemorrhage_diagnosis_array = hemorrhage_diagnosis_df[batch*100:]._get_values

    # reading images
    print(hemorrhage_diagnosis_array.shape[0])
    ...
    for sNo in tqdm(range(0+49, numSubj+49)):
        ...
        for sliceI in range(0, sliceNos.size):
            ...
            img = imread(img_path)
            x = resize(img, new_size)
            AllCTscans[counterI] = x
            imsave(image_path / (str(counterI+batch*100) + '.png'), x)
            ...
            if os.path.exists(str(segment_path)):
                ...
                imsave(label_path / (str(counterI+batch*100) + '.png'), x)
            else:
